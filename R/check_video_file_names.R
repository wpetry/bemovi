...
check_video_file_names <- function(to.data, raw.video.folder,
  ...
  file.sample.info <- data.table::as.data.table(read.table(paste(to.data, video.description.folder, video.description.file, sep = ""), sep = "\t", colClasses = col_classes, header = TRUE))
